...
| `BASE_LOG_PATH`      | Путь логгов. Если не указывать - логгирование в файл не включится        | `var/log/mock_server`  |
...
- `uvicorn` — ASGI-сервер
- `starlette` - Веб-фреймврок
- `pydantic` — Класс настроек для получения конфигурации из `.env`

### 3. **Логика работы**
- Все запросы направляются в один общий маршрут `/`.
- Сервер анализирует метод, заголовки, тело и параметры.
- Возвращает JSON-ответ с параметрами запроса, ошибку или файл в зависимости от настроек.
- Можно добавить задержку для имитации медленного соединения.
- Можно удалить заголовок Content-Type из стандартного ответа `fastapi`
